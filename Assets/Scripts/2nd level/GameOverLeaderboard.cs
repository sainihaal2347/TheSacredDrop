...
public class GameOverLeaderboard : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        Debug.Log("Game Over panel enabled, updating leaderboard");
    ...
    IEnumerator FetchAndDisplayLeaderboard()
    {
        ...
        if (leaderboardText == null)
        {
            ...
            if (leaderboardText == null)
            {
                Debug.LogError("No TextMeshPro component found in game over panel!");
                yield break;
            }
            else
            {
                Debug.Log("Found TextMeshPro: " + leaderboardText.name);
            }
        }
